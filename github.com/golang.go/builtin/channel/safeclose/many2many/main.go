			...
			for {
				...
				if value == 0 {
					...
					case toStop <- "sender#" + id:
						log.Printf("sender[%s] send stop signal\n", id)
					default:
					}
					return
				}
				...
				select {
				case <-stopCh:
					log.Printf("first select. sender[%s] done\n", id)
					return
				default:
				}
				...
				select {
				case <-stopCh:
					log.Printf("second select. sender[%s] done\n", id)
					return
				case dataCh <- value:
				}
			}
	...
	for i := 0; i < NumReceivers; i++ {
		go func(id string) {
			...
			for {
				...
				select {
				case <-stopCh:
					log.Printf("first select. receiver[%s] done\n", id)
					return
				default:
				}
				...
				select {
				case <-stopCh:
					log.Printf("second select. receiver[%s] done\n", id)
					return
				case value := <-dataCh:
					if value == MaxRandomNumber-1 {
						...
						case toStop <- "receiver#" + id:
							log.Printf("receiver[%s] send stop signal\n", id)
						default:
						}
						return
					}
